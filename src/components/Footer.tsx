...
export default function Footer() {
  ...
  useEffect(() => {
    fetch('/api/homepage')
      .then((r) => r.json())
      .then((d: HomeData) => setGreeting(d.greeting))
      .catch(() => setGreeting(globalConfig.site.author));
  }, []);

  const displayName = greeting || globalConfig.site.author;
  const copyrightText = `© 2025 ${displayName}. All rights reserved`;

  return (
    <section className="mt-auto text-gray-700 bg-white border-t dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700">
      <div className="container flex flex-col items-center justify-center min-h-[40px] mx-auto w-full max-w-7xl px-7 py-4 sm:flex-row sm:min-h-[50px]">
        <Link href="/" className="h-5 text-base group relative z-30 flex items-center space-x-1.5 text-black dark:text-white font-semibold">
          ...
          <span className="-translate-y-0.5">{displayName}</span>
        </Link>
        <p className="mt-2 text-sm text-neutral-700 dark:text-neutral-100 sm:ml-4 sm:pl-4 sm:border-l sm:border-neutral-300 dark:sm:border-neutral-700 sm:mt-0">
          {copyrightText} by <a href="https://github.com/sudharshan1909" target="_blank" rel="noopener noreferrer" className="text-neutral-500 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white font-semibold">sudharshan K</a> <br />
